...
namespace MoneyMaker
{
    public class Monthly_Expenses
    {
        ...
        {
            int payment1 = Payments;
            if (Payments != -1)
            {
                payment1--;
            }
            string com = "insert into Monthly_Expenses (User_ID, Averaged_Price, Product, Num_Payments, Total_Payed, Type_ID, Date_OA, Date_OU, PayMents, Type_Group, Type_Item) VALUES (" + userid + "," + averaged_price + ",'" + product + "', " + num_payments + ", " + total_payed + ", " + product_type + ", '" + date + "', '" + date + "', " + payment1 + ", '" + typegroup + "', '" + typestring + "')";
            oledbhelper.Execute(com);
            Thread.Sleep(500);
            com = "SELECT Monthly_Expenses.ID FROM Monthly_Expenses WHERE(((Monthly_Expenses.User_ID) = " + userid + ") AND((Monthly_Expenses.Averaged_Price) = " + averaged_price + ") AND((Monthly_Expenses.Product) = '" + product + "') AND((Monthly_Expenses.Num_Payments) = " + num_payments + ") AND((Monthly_Expenses.Total_Payed) = " + total_payed + ") AND((Monthly_Expenses.Type_ID) = " + product_type + ") AND((Monthly_Expenses.Date_OA) = '" + date + "') AND((Monthly_Expenses.PayMents) = " + Payments + "))";
        ...
        public static void change(int id, int averaged_price, string product, int num_payments, int total_payed, int product_type, string dateou, int payments, string typegroup, string typestring)
        {
            ...
            if (product != "")
            {
                ...
                com += " Monthly_Expenses.Product = '" + product + "'";
                i = 1;
            }
            if (num_payments != -1)
            {
            ...
            if (product_type != -1)
            {
                ...
                com += " Monthly_Expenses.Type_ID = " + product_type + "";
            }
            if (dateou != "")
            {
                if (i == 1)
        ...
        public static void repeat(int id, string date, int addprice, bool ispayment)
        {
            if (ispayment)
            {
                ...
                string product = rows["Product"].ToString();
                int product_type = int.Parse(rows["Type_ID"].ToString());
        ...
        public static DataTable Getmoutcome(int userid, string product, int money, int type, string date1, string date2, int payments, string typegroup, string typestring)
        {
            ...
            if (product != "")
            {
                com += " AND((Monthly_Expenses.Product) like '%" + product + "%')";
            }
            if (money != -1)
            {
            ...
            if (type != -1)
            {
                com += " AND((Monthly_Expenses.Type_ID) = " + type + ")";
            }
            if (date1 != "")
            {
                if (date2 != "")
